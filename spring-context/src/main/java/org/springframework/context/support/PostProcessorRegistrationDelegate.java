...
final class PostProcessorRegistrationDelegate {
	...
	public static void invokeBeanFactoryPostProcessors(
		...
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			// 注意区分两个接口，BeanDefinitionRegistryPostProcessor是BeanFactoryPostProcessor的子集
			/* 【BeanFactoryPostProcessor】主要针对的操作对象是【BeanFactory】，而【BeanDefinitionRegistryPostProcessor】主要针对的操作对象是BeanDefinition */
			// 存放BeanFactoryPostProcessor的集合
			List<BeanFactoryPostProcessor> regularPostProcessors = new ArrayList<>();
			// 存放BeanDefinitionRegistryPostProcessor的集合
			List<BeanDefinitionRegistryPostProcessor> registryProcessors = new ArrayList<>();

			// 首先处理 入参 【beanFactoryPostProcessors】，遍历处理，将BeanDefinitionRegistryPostProcessor和BeanFactoryPostProcessor区分开
			for (BeanFactoryPostProcessor postProcessor : beanFactoryPostProcessors) {
				// 如果是【BeanDefinitionRegistryPostProcessor】
				if (postProcessor instanceof BeanDefinitionRegistryPostProcessor) {
					BeanDefinitionRegistryPostProcessor registryProcessor =
							(BeanDefinitionRegistryPostProcessor) postProcessor; // 类型转换
			...
			// 找到所有实现BeanDefinitionRegistryPostProcessor接口bean的beanName，此处需要重复查找的原因在于的执行过程中可能会新增其他的BeanDefinitionRegistryPostProcessor
			...
			/** 最后调用所有剩下的【BeanDefinitionRegistryPostProcessors】 */
			...
			/** 如果beanFactory不归属BeanDefinitionRegistry类型，那么直接执行postProcessBeanFactory方法 */
			invokeBeanFactoryPostProcessors(beanFactoryPostProcessors, beanFactory);
		}

		/** ================================================================================================================================================== */
		/** 到这里为止，入参beanFactoryPostProcessors和容器中的所有BeanDefinitionRegistryPostProcessor已经全部处理完毕，下面开始处理容器中所有的【BeanFactoryPostProcessor】 */
		/** ================================================================================================================================================== */

		/* 可能会包含一些实现类，只实现了【BeanFactoryPostProcessor】,并没有实现BeanDefinitionRegistryPostProcessor接口 */
		// Do not initialize FactoryBeans here: We need to leave all regular beans
		// uninitialized to let the bean factory post-processors apply to them!
		...
		/* 用于存放实现【PriorityOrdered】接口的BeanFactoryPostProcessor */
		List<BeanFactoryPostProcessor> priorityOrderedPostProcessors = new ArrayList<>();

		/* 用于存放实现【Ordered】接口的BeanFactoryPostProcessor的beanName */
//		List<String> orderedPostProcessorNames = new ArrayList<>();
		List<BeanFactoryPostProcessor> orderedPostProcessors = new ArrayList<>();

		/* 用于存放【普通】BeanFactoryPostProcessor的beanName */
//		List<String> nonOrderedPostProcessorNames = new ArrayList<>();
		List<BeanFactoryPostProcessor> nonOrderedPostProcessors = new ArrayList<>();

		// 遍历postProcessorNames，将BeanFactoryPostProcessor按实现【PriorityOrdered、实现Ordered、普通】三种类型区分开
		for (String ppName : postProcessorNames) {
			// 跳过已经执行过的BeanFactoryPostProcessor
			if (processedBeans.contains(ppName)) {
				// skip - already processed in first phase above
			}
			// 添加实现了【PriorityOrdered】接口的BeanFactoryPostProcessor到priorityOrderedPostProcessors
			else if (beanFactory.isTypeMatch(ppName, PriorityOrdered.class)) {
				priorityOrderedPostProcessors.add(beanFactory.getBean(ppName, BeanFactoryPostProcessor.class));
			}
			// 添加实现了【Ordered】接口的BeanFactoryPostProcessor的beanName到orderedPostProcessorNames
...
//		 List<BeanFactoryPostProcessor> orderedPostProcessors = new ArrayList<>(orderedPostProcessorNames.size());
		// 遍历实现了Ordered接口的BeanFactoryPostProcessor名字的集合
...
//		sortPostProcessors(orderedPostProcessors, beanFactory);
		sortPostProcessors(orderedPostProcessors,beanFactory);
		// 遍历实现了Ordered接口的BeanFactoryPostProcessor进行排序操作
...
//		invokeBeanFactoryPostProcessors(nonOrderedPostProcessors, beanFactory);
	...
	public static void registerBeanPostProcessors(
		...
		// 找到所有实现了BeanPostProcessor接口的实现类
		...
		// 创建存放没有实现 PriorityOrdered 和 Ordered 接口的BeanPostProcessor的集合
		List<BeanPostProcessor> nonOrderedPostProcessors = new ArrayList<>(nonOrderedPostProcessorNames.size());
		// 遍历集合
		for (String ppName : nonOrderedPostProcessorNames) {
			// 根据ppName找到对应的BeanPostProcessor实例对象
			BeanPostProcessor pp = beanFactory.getBean(ppName, BeanPostProcessor.class);
			// 将没有实现 PriorityOrdered 和 Ordered 接口的BeanPostProcessor添加到nonOrderedPostProcessors集合中
	...
	/**
	 * 调用给定的 BeanFactoryPostProcessor 类型Bean对象
	 *
	...
	private static void registerBeanPostProcessors(
		...
		for (BeanPostProcessor postProcessor : postProcessors) {
			// 将postProcessor添加到beanFactory，它将应用于该工厂创建的Bean，在工厂配置期间调用
	...
	private static final class BeanPostProcessorChecker implements BeanPostProcessor {
		...
		/**
		 * 判断 beanName 是否是完全内部使用
		 * @param beanName
		 * @return
		 */
		private boolean isInfrastructureBean(@Nullable String beanName) {
			// 如果beanName不为null && beanFactory包含具有beanName的beanDefinition对象
			if (beanName != null && this.beanFactory.containsBeanDefinition(beanName)) {
				// 从beanFactory中获取beanName的BeanDefinition对象
				BeanDefinition bd = this.beanFactory.getBeanDefinition(beanName);
				// 如果beanDefinition的角色完全内部使用，返回true，否则返回false
