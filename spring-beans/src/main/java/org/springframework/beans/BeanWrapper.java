...
public interface BeanWrapper extends ConfigurablePropertyAccessor { // spring中的核心接口，是spring中的一个包装类，具有单独或者批量获取和设置属性值，获取属性描述符一级查询属性可读可写的能力，还可以完成类型的转换
	 ...
	 * @since 4.1
	 */
	void setAutoGrowCollectionLimit(int autoGrowCollectionLimit); // 指定数组和集合自动增长的限制，在普通的beanWrapper上，默认值是无限的
	 ...
	 */
	Object getWrappedInstance(); // 返回此对象包装的bean实例

	/**
	 * Return the type of the wrapped bean instance.
	 */
	Class<?> getWrappedClass(); // 返回包装的bean实例的类型
	 ...
	 */
	PropertyDescriptor[] getPropertyDescriptors(); // 返回包装类的属性描述
